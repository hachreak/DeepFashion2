...
def load_gt_query(query_name):
    ...
    # the number of all query clothing items
    query_num = len(np.where(query_style_all > 0)[0])
    # image ids of query clothing items
...
def load_gt_gallery(gallery_name):
...
def evaluate(results, gt_query, gt_gallery, thresh=0.5):
    ...
    # the number of query items that fail to be detected
    miss_num = 0

    for id in query_id_real:
        results_id_ind = np.where(results_image_id_all == id)[0]
        # in case no clothing item is detected
        if len(results_id_ind) == 0:
            continue
        # all query items in the given image
        query_id_ind = np.where(query_image_id_all == id)[0]
        ...
        # assign a ground truth label to each detected clothing item
        iou_ind = np.argmax(iou_id, axis=1)

        for id_ind in range(0, len(query_id_ind)):
            style = query_id_style[id_ind]
            cls = query_id_cls[id_ind]
            # For a given ground truth query item, select a detected item on
            # behalf of it:
            # First find out all detected items which are assigned the given
            # ground truth label
            # and are classified correctly.
            # Then select the detected item with the highest score among these
            # detected items.
            if style > 0:
                ...
                results_style_ind = np.intersect1d(
                    results_style_ind1, results_style_ind2)
                if len(results_style_ind) > 0:
                    ...
                    results_style_gallery_id = results_id_gallery_id[
                        results_style_query_ind]
                    results_style_gallery_box = results_id_gallery_box[
                        results_style_query_ind]

                    # find out the corresponding ground truth items in the
                    # gallery, that is ground truth items which have the same
                    # pair id and style as the query item.
                    gt_gallery_ind1 = np.where(gallery_pair_all == pair_id)[0]
                    gt_gellery_ind2 = np.where(gallery_style_all == style)[0]
                    gt_gallery_ind = np.intersect1d(
                            gt_gallery_ind1,
                            gt_gellery_ind2)
                    ...
                    # calculate top-1
                    for t in range(0, 1):
                        # if corresponding ground truth gallery images contains
                        # retrieved gallery image,
                        # first find out the exact corresponding ground truth
                        # gallery image,
                        # then find out ground truth gallery items in this
                        # ground truth gallery image(whose number may be
                        # greater than 1) if the overlap between the retrieved
                        # gallery item and one of the ground truth gallery
                        # items is over the thresh, the retrieved result
                        # is positive.
                        if results_style_gallery_id[t] in gt_gallery_image_id:
                            which_ind = np.where(
                                gt_gallery_image_id ==
                                results_style_gallery_id[t]
                            )[0]
                            crowd = np.zeros(len(which_ind))
                            iou_style = maskUtils.iou([
                                results_style_gallery_box[t]],
                                gt_gallery_box[which_ind], crowd)
