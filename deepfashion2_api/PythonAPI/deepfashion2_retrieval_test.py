    ...
    for id_ind in range(0, len(query_id_ind)):
        ...
        if style > 0:
            ...
            if len(results_style_ind) > 0:
                ...
                # find out the corresponding ground truth items in the gallery,
                # that is ground truth items which have the same pair id and
                # style as the query item.
                ...
                for t in range(0, 1):
                    ...
                    # gallery image,
                    # then find out ground truth gallery items in this ground
                    # truth gallery image(whose number may be greater than 1)
                    # if the overlap between the retrieved gallery item and
                    # one of the ground truth gallery items is over the thresh,
                    # the retrieved result is positive.
                    if results_style_gallery_id[t] in gt_gallery_image_id:
                        which_ind = np.where(
                            gt_gallery_image_id == results_style_gallery_id[t]
                        )[0]
